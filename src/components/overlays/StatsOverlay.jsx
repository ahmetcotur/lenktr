...
const StatsOverlay = ({ link, onClose }) => {
    ...
    const metrics = [
        { label: t('statsOverlay.totalClicks'), value: link.clicks, change: '+12%', color: 'blue' },
        { label: t('statsOverlay.uniqueVisitors'), value: '842', change: '+5%', color: 'purple' },
        { label: t('statsOverlay.avgDuration'), value: '45s', change: '-2%', color: 'orange' },
        { label: t('statsOverlay.bounceRate'), value: '24%', change: '-0.5%', color: 'lime' }
    ];
        ...
        <div className="fixed inset-0 z-[60] bg-[#08090D]/95 backdrop-blur-sm overflow-y-auto animate-fade-in">
            ...
            <div className="relative lg:ml-[300px] max-w-7xl mx-auto p-8">
                ...
                <div className="flex items-center justify-between mb-12">
                    <div>
                        <button onClick={onClose} className="text-zinc-500 hover:text-white flex items-center gap-2 mb-2 transition-colors">
                            <ChevronRight size={20} className="rotate-180" /> {t('statsOverlay.backToLinks')}
                        </button>
                        <h1 className="text-4xl font-black text-white uppercase italic tracking-tighter">
                            {t('statsOverlay.title')} <span className="text-lime-500">📊</span>
                        </h1>
                        <p className="text-zinc-500 font-medium">{t('statsOverlay.detailedStats')} <span className="text-white font-bold">{link.short_slug}</span></p>
                    </div>
                ...
                <div className="p-8 rounded-3xl bg-[#0D0F14] border border-white/5 mb-8">
                    <h3 className="text-xl font-bold text-white mb-8">{t('statsOverlay.trafficOverview')}</h3>
                    <div className="h-64 flex items-end justify-between gap-4 px-4">
                        {chartData.map((value, i) => (
                            <div key={i} className="w-full flex flex-col justify-end gap-2 group">
                                ...
                                <span className="text-xs font-mono text-zinc-600 text-center group-hover:text-blue-400 transition-colors">{t('statsOverlay.day')} {i + 1}</span>
                            </div>
                        ))}
                    </div>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                    ...
                    <div className="p-8 rounded-3xl bg-[#0D0F14] border border-white/5">
                        <h3 className="text-xl font-bold text-white mb-6">{t('statsOverlay.topReferrers')}</h3>
                    ...
                    {/* Locations */}
                    <div className="p-8 rounded-3xl bg-[#0D0F14] border border-white/5">
                        <h3 className="text-xl font-bold text-white mb-6">{t('statsOverlay.topLocations')}</h3>
                        <div className="space-y-4">
                            {[
