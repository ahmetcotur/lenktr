        ...
        return (
            <div className="flex items-center justify-center min-h-[60vh]">
                <div className="text-center space-y-4">
                    ...
                    <p className="text-zinc-500 font-bold uppercase tracking-widest text-xs">{t('dashboard.loading')}</p>
                </div>
            </div>
        );
    }
    ...
    return (
        <div className="space-y-8 animate-fade-in">
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 pb-6 border-b border-white/5">
                <div>
                    <h1 className="text-4xl font-black text-white italic tracking-tighter uppercase mb-2">{t('dashboard.title')}</h1>
                    <p className="text-zinc-500 font-medium">{t('dashboard.welcome')}</p>
                </div>
                <div className="flex gap-3">
                    <Button variant="secondary" size="md" onClick={() => navigate('/links')}>
                        <Link2 size={16} className="mr-2" /> {t('dashboard.buttons.manageLinks')}
                    </Button>
                    <Button variant="primary" size="md" glow onClick={() => navigate('/bio')}>
                        <UserCircle size={16} className="mr-2" /> {t('dashboard.buttons.bioPages')}
                    </Button>
                </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                <Card className="p-6 hover:border-blue-500/30 transition-all">
                    <div className="flex items-center justify-between mb-4">
                        ...
                        <Badge variant="success" className="text-xs">
                            <TrendingUp size={12} className="mr-1" /> {t('dashboard.stats.live')}
                        </Badge>
                    </div>
                    <h3 className="text-3xl font-black text-white mb-1">{dashboardData.totalClicks.toLocaleString()}</h3>
                    <p className="text-xs font-bold text-zinc-500 uppercase tracking-wider">{t('dashboard.stats.totalClicks')}</p>
                </Card>
                ...
                <Card className="p-6 hover:border-purple-500/30 transition-all">
                    ...
                    <p className="text-xs font-bold text-zinc-500 uppercase tracking-wider">{t('dashboard.stats.bioViews')}</p>
                </Card>
                ...
                <Card className="p-6 hover:border-lime-500/30 transition-all">
                    ...
                    <p className="text-xs font-bold text-zinc-500 uppercase tracking-wider">{t('dashboard.stats.activeLinks')}</p>
                </Card>
                ...
                <Card className="p-6 hover:border-orange-500/30 transition-all">
                    ...
                    <p className="text-xs font-bold text-zinc-500 uppercase tracking-wider">{t('dashboard.stats.activeBioPages')}</p>
                </Card>
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                ...
                <Card className="p-6">
                    <div className="flex items-center justify-between mb-6">
                        <h2 className="text-xl font-black text-white flex items-center gap-2">
                            <Link2 size={20} className="text-blue-500" /> {t('dashboard.recentLinks.title')}
                        </h2>
                    ...
                    <div className="space-y-3">
                        {dashboardData.recentLinks.length === 0 ? (
                            <div className="text-center py-8 text-zinc-600">
                                ...
                                <p className="text-sm">{t('dashboard.recentLinks.noLinks')}</p>
                                <Button variant="primary" size="sm" className="mt-4" onClick={() => navigate('/links')}>
                                    <Plus size={14} className="mr-2" /> {t('dashboard.recentLinks.createLink')}
                                </Button>
                            </div>
                        ) : (
                                    ...
                                    <div className="flex items-center justify-between">
                                        ...
                                        <div className="flex items-center gap-4 ml-4">
                                            <div className="text-right">
                                                ...
                                                <div className="text-[10px] text-zinc-600 uppercase font-bold">{t('dashboard.recentLinks.clicks')}</div>
                                            </div>
                ...
                <Card className="p-6">
                    <div className="flex items-center justify-between mb-6">
                        <h2 className="text-xl font-black text-white flex items-center gap-2">
                            <UserCircle size={20} className="text-purple-500" /> {t('dashboard.recentBioPages.title')}
                        </h2>
                        <Button variant="ghost" size="sm" onClick={() => navigate('/bio')}>
                            {t('dashboard.recentBioPages.viewAll')}
                        </Button>
                    </div>
                    <div className="space-y-3">
                        {dashboardData.recentBioPages.length === 0 ? (
                            <div className="text-center py-8 text-zinc-600">
                                ...
                                <p className="text-sm">{t('dashboard.recentBioPages.noBioPages')}</p>
                                <Button variant="primary" size="sm" className="mt-4" onClick={() => navigate('/bio/editor')}>
                                    <Plus size={14} className="mr-2" /> {t('dashboard.recentBioPages.createBioPage')}
                                </Button>
                            </div>
                        ) : (
                            dashboardData.recentBioPages.map(page => (
                                <div key={page.id} className="p-4 bg-white/5 rounded-xl hover:bg-white/10 transition-all border border-white/5 hover:border-purple-500/30">
                                    <div className="flex items-center justify-between">
                                        <div className="flex-1 min-w-0">
                                            <h3 className="text-sm font-bold text-white truncate">{page.profile_title || t('dashboard.recentBioPages.untitled')}</h3>
                                        ...
                                        <div className="flex items-center gap-4 ml-4">
                                            <div className="text-right">
                                                ...
                                                <div className="text-[10px] text-zinc-600 uppercase font-bold">{t('dashboard.recentBioPages.views')}</div>
                                            </div>
            ...
            <Card className="p-8 bg-gradient-to-br from-blue-600/10 to-purple-600/10 border-blue-500/20">
                <div className="flex flex-col md:flex-row items-center justify-between gap-6">
                    <div>
                        <h2 className="text-2xl font-black text-white mb-2">{t('dashboard.quickActions.title')}</h2>
                        <p className="text-zinc-400">{t('dashboard.quickActions.description')}</p>
                    </div>
                    <div className="flex gap-3">
                        <Button variant="secondary" size="lg" onClick={() => navigate('/links')}>
                            <Plus size={18} className="mr-2" /> {t('dashboard.quickActions.newLink')}
                        </Button>
                        <Button variant="primary" size="lg" glow onClick={() => navigate('/bio/editor')}>
                            <Plus size={18} className="mr-2" /> {t('dashboard.quickActions.newBioPage')}
                        </Button>
                    </div>
                </div>
            </Card>
