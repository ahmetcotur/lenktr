    ...
    useEffect(() => {
        const fetchAnalytics = async () => {
            ...
            try {
                ...
                const deviceStats = [
                    { title: t('analytics.devices.mobile'), icon: Smartphone, percent: `${Math.round((devicesMap.mobile / logs.length) * 100) || 0}%`, color: 'blue' },
                    { title: t('analytics.devices.desktop'), icon: Monitor, percent: `${Math.round((devicesMap.desktop / logs.length) * 100) || 0}%`, color: 'purple' },
                    { title: t('analytics.devices.tablet'), icon: Tablet, percent: `${Math.round((devicesMap.tablet / logs.length) * 100) || 0}%`, color: 'orange' },
                ];
    ...
    return (
        <div className="space-y-10 animate-fade-in max-w-[1600px] mx-auto pb-20 font-sans">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-6">
                {[
                    { label: t('analytics.kpis.totalViews'), val: stats.totalViews.toLocaleString(), change: '+12.5%', icon: Users, color: 'blue' },
                    { label: t('analytics.kpis.totalClicks'), val: stats.totalClicks.toLocaleString(), change: '+5.2%', icon: MousePointer2, color: 'purple' },
                    { label: t('analytics.kpis.avgCtr'), val: `${stats.avgCtr}%`, change: '+2.1%', icon: Zap, color: 'lime' },
                    { label: t('analytics.kpis.avgTime'), val: '1m 24s', change: '-0.8%', icon: Clock, color: 'orange' },
                ].map((stat, i) => (
            ...
            <div className="grid grid-cols-1 xl:grid-cols-3 gap-8">
                ...
                <Card className="p-0 overflow-hidden flex flex-col bg-[#0D0F14]">
                    <div className="p-8 border-b border-white/5">
                        <h3 className="text-xl font-bold text-white flex items-center gap-3">
                            <Activity size={20} className="text-lime-500" /> {t('analytics.topPerforming.title')}
                        </h3>
                    </div>
                    <div className="flex-1 overflow-y-auto">
                        {stats.topItems.length === 0 ? (
                            <div className="p-10 text-center text-zinc-600 font-bold uppercase tracking-widest text-xs">{t('analytics.noData')}</div>
                    ...
                    <div className="p-4 border-t border-white/5 bg-zinc-900/20">
                        <Button variant="ghost" className="w-full text-xs uppercase tracking-widest text-zinc-500 hover:text-white">{t('analytics.fullAnalytics')}</Button>
                    </div>
                </Card>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                {(stats.deviceStats.length > 0 ? stats.deviceStats : [
                    { title: t('analytics.devices.mobile'), icon: Smartphone, percent: '0%', color: 'blue' },
                    { title: t('analytics.devices.desktop'), icon: Monitor, percent: '0%', color: 'purple' },
                    { title: t('analytics.devices.tablet'), icon: Tablet, percent: '0%', color: 'orange' },
                    { title: t('analytics.devices.other'), icon: Globe, percent: '0%', color: 'gray' },
                ]).map((device, i) => (
